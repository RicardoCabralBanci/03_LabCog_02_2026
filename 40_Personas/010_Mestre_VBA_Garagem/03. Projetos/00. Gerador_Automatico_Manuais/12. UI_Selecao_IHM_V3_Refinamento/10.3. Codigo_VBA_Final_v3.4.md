# Código VBA Final (Versão 3.4 - Clean Grid & Backgrounds)

**Mudanças Visuais**:
1.  Fundo das Zonas A/B/C agora possui cor "Cinza Gelo" para contraste.
2.  Títulos centralizados com precisão (`CenterAcrossSelection`).
...
' ==============================================================================
' ARQUITETURA DE UI - VERSÃO 3.4 (CLEAN BACKGROUNDS)
' ==============================================================================
...
Private Const CLR_SIDEBAR_BG As Long = 2960685    ' RGB(45,45,45) - Cinza Carvão (Sidebar)
...
Public Sub Init_UI()
    ...
    ' 1. Sidebar (A5 até fim)
    ws.Range(ws.Cells(5, 1), ws.Cells(ws.Rows.Count, 1)).Interior.Color = CLR_SIDEBAR_BG
    
    ' 2. Zonas (Cavas de Conteúdo)
    ' Pinta de B5 até P[fim] com a cor Cinza Gelo
    ' Isso cria o fundo suave onde os cards vão flutuar
    ws.Range(ws.Cells(5, 2), ws.Cells(ws.Rows.Count, 16)).Interior.Color = CLR_ZONE_BG
    
    ' 3. Cabeçalho Unificado (A4:P4)
    With ws.Range("A4:P4")
    ...
    ' --- TÍTULOS (CENTRALIZAÇÃO INTELIGENTE) ---
...
Private Sub Draw_Flat_Btn(id As String, label As String, startRow As Integer, isOn As Boolean)
    ...
    ' Margin maior para o botão "respirar" dentro da coluna escura
