# Proposta de Atualização: mod_NewEngine_V6 (Com Correção de Machine_Type)

**Data:** 26/01/2026
**Autor:** Mestre em VBA
**Status:** Proposta de Correção
**Objetivo:** Corrigir o problema onde a Capa do Manual ficava com o "Tipo da Máquina" vazio. Esta versão captura explicitamente o campo `Info.Maquina` (que geralmente contém o nome comercial completo, ex: "Innopal PB 1/2") e o envia para o C# via CSV.

```vba
Option Explicit

' ==========================================================================================
' MÓDULO: mod_NewEngine_V6 (Ponte Geográfica - Correção Machine_Type)
' DESCRIÇÃO: Ponte entre Botões do Excel e Motor C# V2 + Metadados da Capa Corrigidos
' FUNCIONAMENTO: Recebe o tipo da máquina, coleta metadados do Form Info (incluindo Info.Maquina), gera manifesto e chama o EXE.
...
Private Sub ExportarParaNovoGerador(machineType As String)
    ...
    valRevisao = "0"
    valAno = Year(Now)
    valMaquina = machineType ' Default é o código do botão (BTR, CMX, etc) caso o form falhe
    ...
    If Not Info Is Nothing Then
        ...
        ' CORREÇÃO CRÍTICA: Captura o nome comercial da máquina
