...
On Error GoTo 0
    ...
    If Planilha3.Range("B448") = "Yes" Then
    ...
    For i = 446 To 532
        ...
        If Planilha3.Range("B448") = "Yes" Then   'IHM yes + 1 row
            Select Case i
            Case 449 To 483, 510 To 531 'IHM yes + 1 to before Chapter 5 start, After Chapter 5 end to "Contra capa" -1
                'Do nothing
            Case Else: GoTo Generate: End Select: End If
        
        If Planilha3.Range("B448") = "No" Then
...
Generate:
        ...
        If Planilha3.Range("B" & i) = "Yes" Then
            ...
            docrange.Find.Execute findtext:="Quebra de página"
...
LoopEnd:
    ...
    MsgBox "Manual gerado na pasta selecionada, favor revisar assim que possível!", vbExclamation + vbOKOnly, "Revisão pendente": End If
...
CloseEverything:
    ...
    If Not objTempDoc Is Nothing Then objTempDoc.Close
    MsgBox "Infelizmente um erro ocorreu, tente novamente ou entre em contato com o responsável.", vbCritical + vbOKOnly, "Erro  :("
    On Error GoTo 0
    Exit Sub
...
Private Sub UpdateCustomProperties(ByRef objDoc As Word.Document)
    ...
    UpdateCDP "MachineType", objDoc, "PET DMG": UpdateCDP "MachineModel", objDoc, Info.Maquina
...
On Error GoTo hand
    ...
    For Each oTable In objDoc.Tables
            ...
            If oTable.Title = "Dados da máquina" Then
                Set erng = Planilha27.Range("B26:C32")
                erng.Copy
                Wait (1)
            ...
            If oTable.Title = "Dados - Rinser" Then
                Set erng = Planilha27.Range("B12:D15")
                erng.Copy
                Wait (1)
                ...
                For i = 1 To .Cells.Count
                    If .Cells(i).ColumnIndex > 1 Then
                        ...
                        ntable.PreferredWidthType = wdPreferredWidthPercent
                        .Cells(i).Column.PreferredWidth = 25
                    End If
                Next
            ...
            If oTable.Title = "Dados - Enchedora" Then
                Set erng = Planilha27.Range("B6:D9")
                erng.Copy
                Wait (1)
            ...
            If oTable.Title = "Dados - Lacrador" Then
                Set erng = Planilha27.Range("B18:D22")
                erng.Copy
                Wait (1)
            ...
            If oTable.Title = "Dados de enchimento" Then
                Set erng = Planilha27.Range("F2:H22")
                erng.Copy
                Wait (1)
